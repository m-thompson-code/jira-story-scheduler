...
export const getScheduleFromCsv = (csvString: string, developers: number): { issueMap: IssueMap, epicMap: EpicMap, buckets: Bucket[] } => {
    const issues = fromCsv(csvString);

    const { issueMap, epicMap }  = moo2(issues);

    const buckets = getBuckets(issueMap, epicMap, developers);

    return { issueMap, epicMap, buckets };
}

export const getScheduleFromJson = (rawIssues: RawIssue[], developers: number): { issueMap: IssueMap, epicMap: EpicMap, buckets: Bucket[] } => {
